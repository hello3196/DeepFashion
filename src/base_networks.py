...
class UnetSkipConnectionBlock(nn.Module):
    def __init__(self, outer_nc, inner_nc, input_nc=None,
        ...
        downrelu_1 = nn.LeakyReLU(0.2, True)
        downconv_1 = nn.Conv2d(inner_nc, inner_nc, kernel_size=1, stride=1, padding=0, bias=use_bias)
        downnorm_1 = norm_layer(inner_nc)

        down1 = [downrelu_1, downconv_1, downnorm_1]
        ...
        if outermost:
            ...
            if use_dropout:
                model = down + down1 + [submodule] + up + [nn.Dropout(0.5)]
            else:
                model = down + down1 + [submodule] + up
